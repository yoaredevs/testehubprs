...
--// Wind UI
local WindUI = loadstring(game:HttpGet("https://github.com/FootagesfromWind-Up/WindUI/raw/main/dist/main.lua"))()
...
local function startFly()
    ...
    flyConn = RunService.RenderStepped:Connect(function()
        ...
        if isMobile then
            -- MOBILE: direcional virtual (thumbstick)
...
-- Keybinds SO no PC (mobile usa os toggles / botao nativo da WindUI)
...
-- FIX: limpa fly/noclip ao respawnar
    ...
    pcall(function()
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= LocalPlayer and plr.Team ~= LocalPlayer.Team and plr.Character then
                local part = plr.Character:FindFirstChild(cfg.SilentHitbox) or plr.Character:FindFirstChild("HumanoidRootPart")
                local thum = plr.Character:FindFirstChildOfClass("Humanoid")
                if part and thum and thum.Health > 0 then
...
-- FIX: so intercepta FindPartOnRay (armas). NUNCA Raycast generico (evita recursao = crash)
        ...
        if cfg.SilentAim then
            ...
            if method == "FindPartOnRay" or method == "FindPartOnRayWithIgnoreList" then
                local ok, result = pcall(function()
                    ...
                    if target and (math.random(1, 100) <= cfg.SilentChance) then
                        local part = target.Character:FindFirstChild(cfg.SilentHitbox)
...
--                  LOOPS DE COMBATE
-- ============================================================
-- FIX: Kill Aura com throttle (antes ativava TODO frame = risco de kick)
local lastAura = 0
...
--                 HITBOX EXPANDER
-- ============================================================
-- FIX: salva E restaura tamanho + transparencia ORIGINAIS
local hbOriginal = {}
...
--                     GUN MODS
-- ============================================================
-- FIX: sem .Changed infinito (acumulava conexoes)
task.spawn(function()
    while true do
        task.wait(0.15)
        if cfg.NoRecoil or cfg.NoSpread or cfg.InfAmmo or cfg.FastFire then
            local char = getChar()
            local tool = char and char:FindFirstChildOfClass("Tool")
            if tool then
...
-- FOV Circle: SO no PC e so se a Drawing API existir (FIX de crash no mobile)
local fovCircle = nil
...
RunService.RenderStepped:Connect(function()
    if fovCircle then
        ...
        if not ok then fovCircle = nil end -- destravou de vez se der erro
    end
end)
...
-- ============================================================
-- FIX: BillboardGui dentro de Tool nao renderiza! Agora vai no Handle/BasePart
local itemEspObjects = {}
local function updateItemESP()
    for _, h in pairs(itemEspObjects) do pcall(function() h:Destroy() end) end
    table.clear(itemEspObjects)
    if not cfg.ItemESP then return end
    for _, obj in ipairs(Workspace:GetChildren()) do
        pcall(function()
            if obj:IsA("Tool") and obj.Parent == Workspace then -- so itens largados no chao
                ...
                if handle then
                    ...
                    table.insert(itemEspObjects, gui)
                end
            end
        end)
    end
...
--                       ITENS
-- ============================================================
local function giveItemSafe(name)
    local ok = pcall(function()
        local remote = Workspace:FindFirstChild("Remote")
        if remote and remote:FindFirstChild("ItemHandler") then
            remote.ItemHandler:FireServer(name)
        else
            local giver = Workspace:FindFirstChild("Prison_ITEMS")
            giver = giver and giver:FindFirstChild("giver")
            local item = giver and giver:FindFirstChild(name)
            local part = item and (item:FindFirstChild("ITEMS") or item:FindFirstChildOfClass("BasePart"))
            local hrp = getRoot()
            if part and hrp then
                ...
                error("giver nao encontrado")
            end
        end
    end)
    WindUI:Notify({
        Title = "Itens",
        Content = ok and ("Pegando: " .. name) or ("Falha ao pegar " .. name .. " (voce esta na cela?)"),
        Duration = 3,
...
-- ============================================================
--             TELEPORTE SEGURO (corrigido)
-- ============================================================
-- FIX: zera velocidade FISICA real, pausa fly durante o TP, cooldown anti-spam
local lastTP = 0
local function safeTeleport(cf)
    ...
    if os.clock() - lastTP < 0.5 then return false end -- anti-spam
    lastTP = os.clock()
    ...
    -- fixa por alguns frames pro servidor aceitar
    local holdConn
...
local function setTeam(teamName)
    ...
    pcall(function()
        local remote = Workspace:FindFirstChild("Remote")
        if remote and remote:FindFirstChild("TeamHandler") then
            remote.TeamHandler:FireServer(teamName)
            ok = true
        end
    end)
    if not ok then
    ...
    if not ok and teamName == "Criminals" then
        local hrp = getRoot()
        if hrp then hrp.CFrame = CFrame.new(-423, 46, -162) end
    end
    WindUI:Notify({
        Title = "Team Switcher",
        Content = ok and ("Trocando para: " .. teamName) or "Remote nao achado, use respawn/morte para trocar.",
        Duration = 5,
...
S1:Slider({ Title = "Velocidade do Fly", Value = 60, Min = 20, Max = 300, Step = 5, Callback = function(v) cfg.FlySpeed = v end })
S1:Toggle({
    Title = "Noclip",
...
S1b:Slider({ Title = "Velocidade", Value = 16, Min = 16, Max = 200, Step = 1, Callback = function(v) cfg.Speed = v end })
S1b:Slider({ Title = "Forca do Pulo", Value = 50, Min = 50, Max = 300, Step = 10, Callback = function(v) cfg.Jump = v end })
...
S3:Slider({ Title = "Alcance (studs)", Value = 12, Min = 5, Max = 40, Step = 1, Callback = function(v) cfg.AuraRange = v end })

local S4 = TabCombat:Section({ Title = "Aimbot" })
S4:Toggle({ Title = "Aimbot", Value = false, Callback = function(v) cfg.Aimbot = v end })
S4:Slider({ Title = "FOV", Value = 120, Min = 20, Max = 600, Step = 10, Callback = function(v) cfg.AimFOV = v end })
S4:Slider({ Title = "Suavidade", Value = 0.4, Min = 0.05, Max = 1, Step = 0.05, Callback = function(v) cfg.AimSmooth = v end })
...
S13:Slider({ Title = "Chance de Acerto (%)", Value = 100, Min = 10, Max = 100, Step = 5, Callback = function(v) cfg.SilentChance = v end })
S13:Dropdown({ Title = "Hitbox Alvo", Values = { "Head", "HumanoidRootPart", "Torso" }, Selected = "Head", Callback = function(v) cfg.SilentHitbox = v end })
...
S5:Slider({ Title = "Tamanho da Hitbox", Value = 10, Min = 3, Max = 30, Step = 1, Callback = function(v) cfg.HitboxSize = v end })
S5:Slider({ Title = "Transparencia", Value = 0.8, Min = 0, Max = 1, Step = 0.1, Callback = function(v) cfg.HitboxTrans = v end })
...
S7:Button({ Title = "Keycard", Callback = function() giveItemSafe("Keycard") end })
...
S14:Slider({ Title = "Delay do Fast Fire (s)", Value = 0.1, Min = 0.05, Max = 0.5, Step = 0.05, Callback = function(v) cfg.FireDelay = v end })

--// ---------- TELEPORTES (corrigido) ----------
...
S8:Dropdown({ Title = "Local", Values = tpKeys, Selected = tpKeys[1], Callback = function(v) selectedTP = v end })
...
-- Meus Waypoints (fix: se coordenada vier errada, o usuario salva a certa)
...
wpDropdown = S8b:Dropdown({ Title = "Waypoints Salvos", Value = "Nenhum", Values = { "Nenhum" }, Callback = function(v)
...
-- TP para jogador (fix: atras do alvo, elevado, olhando pra ele)
...
S10:ColorPicker({ Title = "Cor do Fill (desativa cores por time)", Default = Color3.fromRGB(120, 120, 255), Callback = function(c)
    ESPFillColor = c
...
local S16 = TabMisc:Section({ Title = "Team Switcher", Desc = "Pode exigir respawn para aplicar" })
