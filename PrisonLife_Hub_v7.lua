--[[
    ╔═════════════════════════════════════════════╗
    ║   PRISON LIFE HUB v7 | Wind UI              ║
    ║   Mobile + PC | Reescrito do zero           ║
    ╚═════════════════════════════════════════════╝
]]
...
do
    local ok, err = pcall(function()
        WindUI = loadstring(game:HttpGet("https://github.com/Footagesus/WindUI/raw/main/dist/main.lua"))()
    end)
    if not ok or type(WindUI) ~= "table" then
        error("[PLHub] Falha ao carregar a WindUI:\n" .. tostring(err))
    end
end
...
--// Proteção de UI: nenhuma secao quebra o script inteiro
local function UISafe(name, fn)
    local ok, err = pcall(fn)
    if not ok then
        warn("[PLHub] Erro em '" .. name .. "': " .. tostring(err))
        pcall(function()
            WindUI:Notify({ Title = "Erro de UI", Content = name .. ": " .. tostring(err), Duration = 8, Type = "Warning" })
        end)
    end
end
...
local function notify(title, content, ntype)
    pcall(function()
        WindUI:Notify({ Title = title, Content = content, Duration = 4, Type = ntype or "Info" })
    end)
end

-- ============================================================
--  ITENS  (busca recursiva + InvokeServer protegido)
-- ============================================================
local giveItemSafe
do
    ...
    giveItemSafe = function(name)
        local ok = tryPickup(name)
        notify("Itens", ok and ("Pegando: " .. name)
            or ("Falha ao pegar " .. name .. " (nao existe, foi removido ou precisa de gamepass)"),
            ok and "Info" or "Warning")
    end
end

-- ============================================================
--  STATUS (velocidade / pulo)
-- ============================================================
task.spawn(function()
    while true do
        task.wait(0.25)
...
-- ============================================================
--  SILENT AIM  (alvo 1x por frame + hook estendido)
-- ============================================================
            ...
            if part then
                ...
                elseif method == "Raycast" then
                    ...
                    if typeof(origin) == "Vector3" and typeof(direction) == "Vector3" then
                        args[2] = (part.Position - origin).Unit * direction.Magnitude
                        return OldNamecall(self, unpack(args))
                    end
                end
            end
...
-- ============================================================
--  COMBATE (anti-stun / kill aura / aimbot)
-- ============================================================
local lastAura = 0

RunService.Heartbeat:Connect(function()
    -- Anti-Stun (taser/ragdoll)
    ...
    if cfg.KillAura and (os.clock() - lastAura) > 0.15 then
        ...
        if root and tool and hum and hum.Health > 0 then
            local target, best = nil, cfg.AuraRange + 1
            for _, plr in ipairs(Players:GetPlayers()) do
    ...
    if gs then
        pcall(function()
            ...
            if type(module) == "table" then
                ...
                if cfg.FastFire then
                    if module.FireRate then module.FireRate = cfg.FireDelay end
                    if module.ReloadTime then module.ReloadTime = 0.0001 end
                end
            end
        end)
    end
    ...
    for _, obj in ipairs(tool:GetDescendants()) do
        pcall(function()
            if obj:IsA("NumberValue") or obj:IsA("IntValue") then
                if cfg.NoRecoil and (obj.Name == "Recoil" or obj.Name == "RecoilAmount") then obj.Value = 0 end
                if cfg.NoSpread and (obj.Name == "Spread" or obj.Name == "MaxSpread") then obj.Value = 0 end
                if cfg.NoSpread and obj.Name == "Accuracy" then obj.Value = 100 end
                if cfg.FastFire and (obj.Name == "FireRate" or obj.Name == "RateOfFire"
                    or obj.Name == "Cooldown" or obj.Name == "FireDelay") then obj.Value = cfg.FireDelay end
                if cfg.InfAmmo and (obj.Name == "Ammo" or obj.Name == "CurrentAmmo" or obj.Name == "Magazine") then
                    if obj.Value < 3 then obj.Value = obj.MaxValue or 20 end
                end
            end
        end)
    end
...
local function safeTeleport(cf)
    ...
    local hrp = getRoot()
    if not hrp then
        notify("Teleporte", "Personagem nao encontrado! Espere o respawn.", "Warning")
        return false
    end
...
local function serverHop()
    local requestFn = (syn and syn.request) or request or http_request or (http and http.request)
    if not requestFn then
        notify("Erro", "Seu executor nao suporta HTTP.", "Error")
        return
    end
    ...
    if not ok or not body then
        notify("Erro", "Falha ao buscar servidores.", "Error")
        return
    end
...
local function fpsBoost()
    ...
    notify("FPS Booster", "Graficos otimizados!", "Success")
end

local function setTeam(teamName)
    local remote = Workspace:FindFirstChild("Remote")
    local ok = false
    if remote and remote:FindFirstChild("TeamEvent") then
        ...
        if ok and remote:FindFirstChild("loadchar") then
            pcall(function()
                task.wait(0.5)
                remote.loadchar:InvokeServer(LocalPlayer.Name)
            end)
        end
    end
    if teamName == "Criminals" then
        -- nao existe evento: teleporta para a base criminosa
        local spawn = Workspace:FindFirstChild("Criminals Spawn", true)
        if spawn then
            local part = spawn:IsA("BasePart") and spawn or spawn:FindFirstChildWhichIsA("BasePart", true)
            if part then
                ok = safeTeleport(part.CFrame + Vector3.new(0, 5, 0))
            end
        end
        if not ok then
    ...
    notify("Team Switcher", ok and ("Trocando para: " .. teamName) or "Falha ao trocar de time.",
...
--// ---------- JOGADOR ----------
UISafe("Jogador/Movimento", function()
    local S1 = TabPlayer:Section({ Title = "Movimento" })
    S1:Toggle({
        Title = "Fly",
        Desc = isMobile and "Mova com o direcional" or "WASD + E/Q | tecla V",
        Value = false,
        Callback = function(v)
            if v then startFly() else stopFly() end
        end
    })
    S1:Slider({ Title = "Velocidade do Fly", Value = { Default = 60, Min = 20, Max = 300 }, Step = 5,
...
UISafe("Jogador/Status", function()
    local S1b = TabPlayer:Section({ Title = "Status" })
    S1b:Toggle({ Title = "Aplicar Velocidade/Pulo", Value = true, Callback = function(v)
        cfg.ApplyStats = v
        if not v then
            local hum = getHum()
            if hum then
    ...
    S1b:Slider({ Title = "Velocidade", Value = { Default = 16, Min = 16, Max = 200 }, Step = 1,
        Callback = function(v) cfg.Speed = v end })
    S1b:Slider({ Title = "Forca do Pulo", Value = { Default = 50, Min = 50, Max = 300 }, Step = 10,
        Callback = function(v) cfg.Jump = v end })
    S1b:Toggle({ Title = "Pulo Infinito", Value = false, Callback = function(v) cfg.InfJump = v end })
end)

UISafe("Jogador/Utilidades", function()
    local S2 = TabPlayer:Section({ Title = "Utilidades" })
    S2:Toggle({ Title = "Anti-Stun / Anti-Taser", Value = false, Callback = function(v) cfg.ActiveAntiStun = v end })
end)

--// ---------- COMBATE ----------
UISafe("Combate/KillAura", function()
    local S3 = TabCombat:Section({ Title = "Kill Aura" })
    S3:Toggle({ Title = "Kill Aura", Desc = "Segure uma faca/arma e ataque automatico", Value = false,
        Callback = function(v) cfg.KillAura = v end })
    S3:Slider({ Title = "Alcance (studs)", Value = { Default = 12, Min = 5, Max = 40 }, Step = 1,
        Callback = function(v) cfg.AuraRange = v end })
end)

UISafe("Combate/Aimbot", function()
    local S4 = TabCombat:Section({ Title = "Aimbot" })
    S4:Toggle({ Title = "Aimbot", Value = false, Callback = function(v) cfg.Aimbot = v end })
    S4:Slider({ Title = "FOV", Value = { Default = 120, Min = 20, Max = 600 }, Step = 10,
        Callback = function(v) cfg.AimFOV = v end })
    S4:Slider({ Title = "Suavidade", Value = { Default = 0.4, Min = 0.05, Max = 1 }, Step = 0.05,
        Callback = function(v) cfg.AimSmooth = v end })
    S4:Toggle({ Title = "Mostrar circulo FOV", Desc = "So no PC", Value = true,
...
UISafe("Combate/SilentAim", function()
    local S13 = TabCombat:Section({ Title = "Silent Aim" })
    S13:Toggle({ Title = "Silent Aim", Desc = "Tiros vao pro alvo mesmo errando", Value = false,
        Callback = function(v) cfg.SilentAim = v end })
    S13:Slider({ Title = "Chance de Acerto (%)", Value = { Default = 100, Min = 10, Max = 100 }, Step = 5,
        Callback = function(v) cfg.SilentChance = v end })
    S13:Dropdown({ Title = "Hitbox Alvo", Values = { "Head", "HumanoidRootPart", "Torso" },
        Value = "Head", Callback = function(v) cfg.SilentHitbox = v end })
end)

UISafe("Combate/Hitbox", function()
    ...
    S5:Slider({ Title = "Tamanho da Hitbox", Value = { Default = 10, Min = 3, Max = 30 }, Step = 1,
        Callback = function(v) cfg.HitboxSize = v end })
    S5:Slider({ Title = "Transparencia", Value = { Default = 0.8, Min = 0, Max = 1 }, Step = 0.1,
        Callback = function(v) cfg.HitboxTrans = v end })
end)

--// ---------- ITENS ----------
UISafe("Itens/Armas", function()
...
UISafe("Itens/GunMods", function()
    ...
    S14:Slider({ Title = "Delay do Fast Fire (s)", Value = { Default = 0.1, Min = 0.05, Max = 0.5 }, Step = 0.05,
        Callback = function(v) cfg.FireDelay = v end })
end)

--// ---------- TELEPORTES ----------
UISafe("TP/Locais", function()
    ...
    S8:Dropdown({ Title = "Local", Values = tpKeys, Value = tpKeys[1],
    ...
    S8:Button({ Title = "Teleportar", Callback = function()
        if selectedTP and teleportes[selectedTP] then
            if safeTeleport(CFrame.new(teleportes[selectedTP])) then
                notify("Teleporte", "Voce foi para: " .. selectedTP)
            end
        end
    end })
end)

UISafe("TP/Waypoints", function()
    ...
    local function rebuildWaypoints()
        ...
        table.sort(wpKeys)
        pcall(function()
            if wpDropdown and wpDropdown.Refresh then
                if #wpKeys == 0 then wpKeys = { "Nenhum" } end
                wpDropdown:Refresh(wpKeys, wpKeys[1])
            end
        end)
    end
    S8b:Button({ Title = "Salvar Posicao Atual", Callback = function()
        ...
        rebuildWaypoints()
        notify("Waypoint", "Salvo: " .. name, "Success")
    end })
    wpDropdown = S8b:Dropdown({ Title = "Waypoints Salvos", Values = { "Nenhum" }, Value = "Nenhum",
        Callback = function(v)
    ...
    S8b:Button({ Title = "Ir para Waypoint", Callback = function()
        if selectedWP and savedWaypoints[selectedWP] then
            if safeTeleport(CFrame.new(savedWaypoints[selectedWP])) then
                notify("Teleporte", "Voce foi para: " .. selectedWP)
            end
        else
            notify("Teleporte", "Nenhum waypoint selecionado!", "Warning")
        end
    end })
end)

UISafe("TP/Jogadores", function()
    ...
    local playerDD
    playerDD = S9:Dropdown({ Title = "Jogador", Values = getPlayerNames(),
        Callback = function(v)
            if v ~= "Nenhum" then selectedPlayer = v end
        end })
    S9:Button({ Title = "Atualizar Lista", Callback = function()
        ...
        notify("Lista", "Lista atualizada!")
    end })
    S9:Button({ Title = "Ir ate o Jogador", Desc = "Aparece atras dele", Callback = function()
        if not selectedPlayer then
            notify("Teleporte", "Selecione um jogador primeiro!", "Warning")
            return
        end
        ...
        if plr and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if safeTeleport(CFrame.lookAt(behindPos, targetHRP.Position)) then
                notify("Teleporte", "Voce foi ate: " .. selectedPlayer)
            end
        else
            notify("Teleporte", "Jogador invalido ou sem personagem!", "Warning")
        end
    end })
end)

--// ---------- VISUAL ----------
UISafe("Visual/ESP", function()
    ...
    S15:Toggle({ Title = "Armas/Facas no chao", Desc = "Destaca itens largados no mapa", Value = false,
        Callback = function(v) cfg.ItemESP = v end })
end)

--// ---------- MISC ----------
UISafe("Misc", function()
...
--// Seleciona a primeira aba (protegido: algumas versoes exigem numero)
if not pcall(function() Window:SelectTab(1) end) then
    pcall(function() Window:SelectTab(TabPlayer) end)
end

notify("Prison Life Hub", "v7 carregado! " .. (isMobile
    and "Use o botao da WindUI (bolinha) para abrir/fechar o menu."
    or "V = Fly | N = Noclip | RightShift = Menu"), "Success")
