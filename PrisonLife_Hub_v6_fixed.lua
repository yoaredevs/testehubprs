...
do
    local function tryPickup(itemName)
        ...
        -- procura tanto em "giver" quanto em "single" (fix: faca fica em single)
        for _, folderName in ipairs({ "giver", "single" }) do
            local folder = prisonItems:FindFirstChild(folderName)
            local item = folder and folder:FindFirstChild(itemName)
            local pickup = item and item:FindFirstChild("ITEMPICKUP")
            if pickup then
                -- ItemHandler eh RemoteFunction: precisa InvokeServer (fix do FireServer)
...
S7:Label({ Text = "Keycard nao tem giver — pegue de guarda abatido." })
...
S16:Label({ Text = "Pode exigir respawn para aplicar" })
